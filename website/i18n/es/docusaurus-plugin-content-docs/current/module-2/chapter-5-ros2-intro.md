...
Procesos autónomos que realizan tareas específicas:```python
import rclpy
...
Comunicación asíncrona y en streaming:```python
from rclpy.node import Node
...
Comunicación síncrona de solicitud-respuesta:```python
...
class ServiceClient(Node):
    ...
    def send_request(self, a, b):
        ...
        return future
```#### 4. Acciones&#10;&#10; Tareas de larga duración con retroalimentación:```python
...
ROS 2 proporciona perfiles de QoS configurables para una comunicación fiable:```python
...
Un paquete típico de ROS 2 Python:```
my_robot_pkg/
...
Lanzar múltiples nodos con configuración:```python
...
```Ejecutar archivo de lanzamiento:```bash
ros2 launch my_robot_pkg robot_launch.py
```## Herramientas de línea de comandos&#10;&#10; Comandos esenciales de la CLI de ROS 2:```bash
# List nodes
ros2 node list
...
```## Ejemplo práctico: Robot Controlador de Velocidad```python
import rclpy
from rclpy.node import Node
...
### 1. Comprobar el estado del nodo```bash
# Is node running?
ros2 node list

# Check node connections
ros2 node info /my_node
```### 2. Supervisar temas```bash
