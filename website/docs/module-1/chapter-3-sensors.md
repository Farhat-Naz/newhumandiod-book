...
A simple yet effective approach for combining accelerometer and gyroscope data:

### Principle

- **Gyroscope**: Accurate short-term, drifts long-term
- **Accelerometer**: Accurate long-term, noisy short-term
- **Solution**: High-pass filter gyro + low-pass filter accelerometer
...
**Prediction**:
- State prediction: `x̂ₖ = A·xₖ₋₁ + B·uₖ`
- Covariance prediction: `Pₖ = A·Pₖ₋₁·Aᵀ + Q`

**Update**:
- Kalman gain: `K = Pₖ·Hᵀ·(H·Pₖ·Hᵀ + R)⁻¹`
- State update: `xₖ = x̂ₖ + K·(zₖ - H·x̂ₖ)`
- Covariance update: `Pₖ = (I - K·H)·Pₖ`
...
Notice how the filter quickly learns the true velocity and smooths out measurement noise!

## Extended Kalman Filter (EKF)

For nonlinear systems, we use the **Extended Kalman Filter**, which linearizes around the current estimate.
...
Modern humanoid robots use deep learning for visual perception:
...
Combining RGB and depth data for 3D perception:
